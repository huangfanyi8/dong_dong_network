...
> **每个目标必须写清「承接痛点」**：它解决了上一个目标的什么问题。这是问题驱动路线的核心，任何目标若无法说清这点，说明顺序或设计有问题。
...
**承接痛点**：目标 02 的 server 是**单线程阻塞**的——`accept()` 只能接一个连接，之后整个程序卡在 `recv()` 上等这个客户端。第二个客户端连进来只能在内核队列排队，第一个客户端一直不发数据，后面的就永远等不到服务。即"**一次只能服务一个客户端**"。

**目标**：改造 server，使每个连接由一个 `std::thread` 处理，多个客户端能同时被服务。
...
- [x] 目标 01：项目骨架 + 认识参考代码
- [x] 目标 02：阻塞式单对单 echo
- [x] 组件：函数式 spdlog 日志系统（Error 枚举 + source_location + 每日文件），作为项目基石先于目标 03 完成 —— commit `f499a51`
- [ ] 目标 03：每连接一线程
- [ ] 目标 04：非阻塞 + 轮询
- [ ] 目标 05：select 多路复用
- [ ] 目标 06：Reactor 雏形
